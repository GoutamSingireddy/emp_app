class UsersController < ApplicationController
  ...
  def index
    if params.has_key?(:order) and params[:order] == 'age'
      ...
      @users =User.order("department")	
	  
	  elsif  params.has_key?(:order) and params[:order]== 'salary'
      @users =User.order("salary")	
	  
	 elsif params.has_key?(:order) and params[:order]=='highest salary'
      @users =User.order("salary desc").first(5)
	  
     elsif params.has_key?(:where) and params[:where]=='retirement'
       @users =User.where("age >59")	
	   
	 elsif params.has_key?(:order) and params[:order]=='salary_department'
      @users =User.order("salary desc").group("Department") 
	  
	 elsif params.has_key?(:order) and params[:order]=='highest age'
      @users =User.order("age desc").first(5)
